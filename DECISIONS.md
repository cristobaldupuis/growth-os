...
**Why a host rather than Thinking Machines directly:** they do have an API — Tinker, their fine-tuning platform, which carries an OpenAI-compatible inference endpoint. It is not the right target for this. That endpoint addresses `tinker://` sampler weight paths (checkpoints from your own training run) rather than a base model by name, and its documentation scopes it to low-traffic internal testing rather than user-facing deployment; Thinking Machines points production inference at Together, Fireworks, Baseten, Modal and Databricks. Tinker is the interesting surface for a *different* question — fine-tuning a model on the portfolio and benching the checkpoint against stock models in the console's test bench — and if that becomes worth doing it is a sibling adapter with the checkpoint path as the catalogue id, not a change to this one.
