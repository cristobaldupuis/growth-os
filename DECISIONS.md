...
**The hazard that is written down rather than hidden:** a performance replace deletes then inserts in chunks, so a chunk that fails leaves fewer rows than it found. It is reported, not silent, and the full set is still in memory for that session. A transactional version needs a staging table and belongs with the fact model in 5.4. *(Closed, September 2026: a replace now stages every chunk under one server-issued `imported_at` batch and deletes the older rows only in a final commit, so a failed chunk leaves too many rows rather than too few. No staging table was needed — see `handlePerfStage` in api/state.js.)*
...
## D-ID is promoted to a tier, for ElevenLabs voice, not for price or quality

**Decision:** D-ID moves from "implemented and reachable but not surfaced" to `VIDEO_TIERS.CUSTOM_VOICE` in `callGenerateVideo.js`. Its `script.provider.type` in `api/video.js` changes from a hardcoded `"microsoft"` to `"elevenlabs"`. Brands gain a plain-text `avatarImageUrl` field (`SettingsView.jsx`, beside `BrandReferences`) — a URL D-ID or Fabric fetch themselves, not an uploaded asset — because both tiers animate a still image and neither has a stock avatar library. Creative Studio resolves `voiceId`/`avatarId` per provider (`videoParamsFor` in `CreativeStudio.jsx`) rather than passing one pair of params through to every provider, because the three providers here read three different things out of the same two argument names.

**Why now, and why not on price.** The previous entry on this provider (see the tier-naming decision above, and `callGenerateVideo.js`'s history) declined to surface it because at ~$0.035/sec it sits above HeyGen without being visibly better. That reasoning still holds on quality. What changed is that D-ID's `/talks` API takes `provider: { type: "elevenlabs", voice_id }` directly — the same ElevenLabs voice id this app already fetches for the "Hear it" audition (`callGenerateVoice.js` / `api/voice.js`) reaches the actual billed render with no second id-mapping step. HeyGen can also carry an ElevenLabs voice, but only after the operator links their ElevenLabs account inside HeyGen's own dashboard (Integrate 3rd Party Voice), which mints a HeyGen-native id this app cannot list — so a HeyGen render takes it as a free-typed `heygenVoiceId`, deliberately kept separate from the ElevenLabs `voiceId` state so the two id spaces are never confused for each other.

**What this exposed, unrelated to ElevenLabs.** Before this change, `genVideo` in `CreativeStudio.jsx` never passed `voiceId` or `avatarId` to `callGenerateVideo` at all. Two consequences, neither previously visible: every real HeyGen render used HeyGen's default voice regardless of what the Voice picker showed (the picker only ever drove the audition, not the render it sits beside), and the PREMIUM tier (VEED Fabric) — which has been selectable since it shipped — could never actually render anything, because `fabric.submit` throws immediately on a missing `avatarId` and nothing in the UI ever supplied one. Premium was a dead button. `avatarImageUrl` on the brand fixes both CUSTOM_VOICE and the pre-existing PREMIUM tier in the same change.

**Why an image URL, not an upload.** `BrandReferences` already uploads to `assetStore.js`, but that path resolves to a blob or data URL for in-app display — bytes D-ID's and Fabric's servers cannot fetch, since they are never given to those servers at all (Nano Banana image generation sends brand references as base64 in the request body; D-ID and Fabric take a URL they retrieve themselves). Reusing that component would silently produce a URL that 404s at generation time. A plain text field puts the burden of hosting on the operator — the same place the video clip's own 24-72h link already puts the burden of downloading it — rather than this app taking on image hosting to solve a problem the asset store was not built for.

**What is not verified.** Whether D-ID's ElevenLabs integration is linked once (in D-ID's Integrations settings) or requires something per-request was not confirmed against D-ID's own dashboard, only against their published API reference and third-party guides — check this against a live D-ID account before relying on it, the same discipline `registry.js` already asks of every `unverified` model id in the text catalogue.

**Forcing condition:** Scene generation (Veo, `api/scene.js`) stays untouched — ElevenLabs has no product that generates a moving frame from a visual direction; its newly-launched Avatars feature is a lip-synced talking head, the same job D-ID and HeyGen already do here, and its API is not yet public at time of writing. If ElevenLabs ships a scene/motion API, that is a new `caps.sceneGen` catalogue entry, not a change to this decision.

---

## The MCP connector is a second, narrower OAuth server, and its tools never leave the ledger

**Decision:** `api/mcp.js` and `api/oauth.js` let a person read and write their own workspace from an MCP client — Claude Desktop, Claude Code, claude.ai, and (via an org-level connector, not a bespoke integration — see below) Claude in Slack — instead of only the browser. The MCP endpoint is a hand-rolled Streamable HTTP JSON-RPC transport rather than `@modelcontextprotocol/sdk`. It sits behind a real OAuth 2.1 authorization server this deployment now also runs: dynamic client registration (RFC 7591), PKCE-only public clients, an authorize step a human completes in a browser by signing in with their existing workspace credentials, and single-use refresh-token rotation with reuse detection. Full schema and reasoning for the AS itself is in `supabase/migrations/0006_mcp.sql`; this entry is about the shape of the surface and its boundary.

Every tool in `api/_mcpTools.js` reads or writes the SAME documents `api/state.js` already serves the browser — `gos_items_v4`, `gos_agenda_v1`, `performance_rows` — through the identical `bump_workspace_doc` revision-checked path. There is no tool that reaches Meta, Google Ads, or any execution surface, and there will not be one added casually.

**Why that boundary, precisely.** "Write access to ad platforms goes through a proposal gate, never a direct tool call" (above) is about spend that is not reversible by editing code — a bad write there costs money immediately, at a rate bounded only by the account's daily cap. Writing your own initiative through Claude is a different risk class entirely: it is the same table, the same person acting, and the same undo (edit it again) as using the form in the browser. Collapsing those two into one worry — "an AI agent can write to my workspace" — would either over-gate the ledger tools with a proposal-diff ceremony they do not need, or, worse, make the ad-platform boundary look like a preference rather than the load-bearing rule it is. The pre-registration gate and the kill-criteria gate apply to an MCP-created initiative exactly as they do to one typed into the form, because the tools import and call the same `validateInitiative`/`killGateBlocked` functions rather than a looser reimplementation — see `api/_mcpTools.js`'s own header.

**Why a second OAuth server instead of handing out the browser's Supabase session.** `api/_auth.js` already answers "whose session is this" for the app, and it would have been less code to let an MCP client present that same session token. Rejected because an MCP client holding it could do everything the browser can, for as long as Supabase's own refresh cycle keeps it alive, revocable only by signing the person out everywhere. Minting our own opaque, workspace-scoped, independently revocable token means pulling a leaked or unwanted connector out later never touches the person's own browser session, and vice versa. The membership check on `verifyAccessToken` reruns on every call rather than being trusted from mint time, for the same reason `api/_auth.js`'s header gives for checking Supabase rather than a local JWT verify: revocation has to actually cut something off, not just stop new grants.

**Why hand-rolled MCP rather than the SDK, when every other provider here is `fetch` and a header.** That habit (`api/_supabase.js`, `api/_geminiAuth.js`) has always been about not adding a *convenience* client for something a few `fetch` calls already do cleanly. MCP's Streamable HTTP transport is not that — it is a wire protocol other people's software has to parse correctly, and the surface this deployment actually needs (`initialize`, `tools/list`, `tools/call`, `ping`, one notification, no server-initiated streaming) is small and fully bounded by the spec rather than open-ended. Getting a protocol detail wrong here is silent breakage against a real client this deployment cannot single-step through in this environment, which is a sharper failure mode than a malformed REST call to a provider whose response shape is forgiving. The call could go either way; it is written down because it is the one place this repo's usual reasoning does not obviously apply, not because the reasoning changed.

**Why CORS is wide open here and nowhere else.** Every other endpoint in `api/` authorises on Origin/Referer against `ALLOWED_ORIGINS` because the caller is meant to be this app's own bundle. The OAuth and MCP endpoints are meant to be called by MCP client infrastructure with no relationship to this app's origin at all — Claude Desktop's local process, claude.ai's backend, an org's Slack connector. Origin checking would simply refuse all of them. What stands in its place is the bearer token itself (opaque, hashed at rest, independently revocable) and PKCE at the authorize step — the same shift api/proxy.js made when `VITE_GOS_SECRET` was retired: origin is a property of a browser tab, not a credential, and it was never the control actually doing the work here either.

**The cost this took from elsewhere.** `scripts/check-functions.mjs` caps this deployment at twelve Serverless Functions on its current plan. Five actions (registration, authorize, token, and two `.well-known` documents) are folded into one file, `api/oauth.js`, dispatched by `?action=`, the same shape `api/admin.js`/`api/video.js`/`api/debate.js` already use — `api/mcp.js` stays separate because it is a genuinely different resource and request shape, not a sibling action. Folding still lands this deployment at exactly twelve of twelve. **There are zero function slots left as of this change.** The next new endpoint needs either a fold into an existing `?action=` set or a plan upgrade decided before the pull request that needs it, per that script's own header on why it exists — this paragraph is that decision made visible rather than rediscovered at deploy time.

**What Slack gets for free, and what was deliberately not built for it.** Once this is a real OAuth-backed remote MCP server, an organisation can add it as a connector to Claude in Slack the same way any org-level MCP connector is added — no bespoke Slack app, no separate OAuth install flow, no second implementation of these tools. That is the reason the remote/OAuth shape was chosen over a simpler local stdio server with a personal access token: a local server would have reached Claude Desktop and Claude Code and nothing else. Actually wiring a Slack connector is an operator action in claude.ai's connector settings, not a code change, and is out of scope here.
