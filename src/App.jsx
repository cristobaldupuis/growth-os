...
import { appendUsage } from "./services/usage.js";
import { probeDurableStorage } from "./services/assetStore.js";
...
export default function App() {
  ...
  // Provenance for every generated frame and render. Records only — see
  // services/assets.js for why the record outliving its bytes is the point
  ...
  useEffect(()=>{
    ...
    // from React state. The ref mirrors the ledger so the sink can append and
    // persist without reading through a stale closure or doing a store write
    // inside a state updater — the latter runs twice under StrictMode.
    onUsage(row => {
      const next = appendUsage(usageRef.current, row);
      usageRef.current = next;
      setUsage(next);
      store.set(KEY_USAGE, JSON.stringify(next));
    });

    // Whether generated bytes will survive a reload. Asked once at boot so the
    // Creative Studio can say so before the operator spends money, rather than
    // after they reload and find the frames gone.
    probeDurableStorage();
  ...
  return (
    <div style={{background:t.bg,minHeight:"100vh",fontFamily:t.sans,color:t.text,
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
              ...
              <button style={{...gG(t),background:t.red,border:"none"}} onClick={()=>{
                ...
                // Records restore; the bytes they point at do not travel in a
                // backup file. Those still in the bucket re-resolve, the rest
      ...
      }} onOpenRec={(batchId,recId)=>setShowRecModal({batchId,recId})} showToast={showToast} onSaveItems={saveItems}/>}
