...
export function PerformanceView({ t, items, settings, perfRows, onImport, onClear, onAssignNames, showToast, initialTab, initialInitiativeId, onOpenConvention }) {
  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 20px 60px" }}>
      ...
      {tab === "convention" && (
        <div style={{ ...gCd(t) }}>
          <div style={{ ...gSL(t), marginBottom: 6 }}>Campaign nomenclature</div>
          <p style={{ fontSize: 13, color: t.textSub, fontFamily: t.sans, lineHeight: 1.6, margin: "0 0 14px", maxWidth: 620 }}>
            The convention is workspace configuration, not a property of this view — it decides how
            every name in the account parses, what the builder can compose, and which dimensions a
            breakdown can pivot on. It lives in Settings with the rest of the things that shape the
            workspace, which is also where the README has always said it lives.
          </p>
          <button style={gG(t)} onClick={onOpenConvention}>
            Open the convention editor <IconArrowRight size={13}/>
          </button>
        </div>
      )}
